...
import { FindLinkSessionByTokenResult } from "@/server/infra/redis/models/results/find-by-token.result";

export interface LinkSessionRepository {
    create(linkSession: LinkSession, ttl: number): Promise<CreateResult>;
    findByToken(token: string): Promise<FindLinkSessionByTokenResult>;
